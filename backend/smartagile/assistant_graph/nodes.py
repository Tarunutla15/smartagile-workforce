...
def load_productivity_node(user: Any, state: AgentState) -> dict[str, Any]:  # noqa: ARG001
    from ..assistant_brain import build_current_productivity_context

    user_text = (state or {}).get("user_text") or ""
    t = user_text.lower()
    is_current = any(k in t for k in ("currently", "right now", "now", "at the moment", "current"))
    ctx = build_current_productivity_context(user, minutes=15) if is_current else build_productivity_context(user)
    ...
    if use_llm and is_llm_configured():
        try:
            system = (
                ...
                "If has_data is false, explain that we need desktop "
                "agent data. If the user asks which app/application they spent most time on, use "
                ...
                "browser_pages.chrome.most_time_in_pages. Important: these are tab/page TITLES, "
                "not guaranteed URLs; say that clearly and list the top 5. "
